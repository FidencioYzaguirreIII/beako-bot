...
async def init_web():
    ...
    try:
        cookieBtn = web.find_element_by_class_name('dl_cookieBanner--buttonClose')
        cookieBtn.click()
...
async def login():
    ...
    login.click()
    web.find_element_by_id("login_email").send_keys(os.getenv('DEEPL_EMAIL'))
    web.find_element_by_id("login_password").send_keys(os.getenv('DEEPL_PASS'))
    login = web.find_element_by_class_name('dl_menu__login__form__submit').click()
